...
//    with efficient (O(1)) lookup, listInsert, and removal of entries
...
class Table {
 public:
   ...
   // listInsert a new pair into the table
 ...
 private:
   ...
   // put private dataLink and headers for private methods here

   unsigned int hashSize;      // listSize of the hash table
   // (used in hashCode method above)

   ListType * dataLink;
   unsigned int numEntry;//number of entries in the HashTable

   int  numFilledBuckets(ListType *data) const;
   //return the number of filled buckets in the HashTable
   int  maxChainLength(ListType *data) const;
   //return the longest chain in the HashTable
